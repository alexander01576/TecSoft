    ...
    <body>
        <nav class="navbar navbar-dark bg-dark">
            <div class="container-fluid">
                <span class="navbar-brand mb-0 h1">Administrador | Gestion de sensores registrados</span>
            </div>
        </nav>
        ...
        <thead class="thead-dark">
            ...
            $sqlCon = "select * from sensor order by id_sensor asc";
            $resul = pg_query($conexion, $sqlCon);
            while ($mostrar = pg_fetch_array($resul)) {

                $id_sensor = $mostrar['id_sensor'];
            ?>
                ...
                <tr>
                    ...
                    <td>
                        <form action="../elim/elim_sensor.php" method="POST">
                            <input type="hidden" name="id_sensor" value=<?php echo $id_sensor ?>>
                            <button type="submit" class="btn btn-danger">eliminar</button>
                        </form>
                    </td>
                    <td>
                        <form action="../actu/actu_sensor.php" method="POST">
                            <input type="hidden" name="id_sensor" value=<?php echo $id_sensor ?>>
