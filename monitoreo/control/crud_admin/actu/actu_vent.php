...
<head>
    ...
    <title>Actualizacion de Ventiladores</title>
</head>
<center>
    ...
    <body background=>
        <nav class="navbar navbar-dark bg-dark">
            <div class="container-fluid">
                <span class="navbar-brand mb-0 h1">Administrador | Actualizacion de ventilador </span>
            </div>
        </nav>

        <?php
        $sqlCon = "select * from ventilador where id_ventilador = $id_vent;";
        $resul = pg_query($conexion, $sqlCon);
        while ($mostrar = pg_fetch_array($resul)) {
        ?>

            <div class="card-body" style="width: 30rem;">
                <form action="ctrl/updt_vent.php" method="POST" name="formDatosPersonas">
                    <input type="hidden" name="idSend" value="<?php echo $id_vent ?>">

                    <h6>Ingresa los datos solicitados</h6>
                    <label>Id de ventilador</label><br>
                    <input required class="form-control" type="number" name="id_ventilador" value="<?php echo $mostrar['id_ventilador'] ?>" placeholder="Id de ventilador" required alert="rellena el id de ventilador" />
                    <br>
